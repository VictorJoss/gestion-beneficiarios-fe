      ...
      <article v-if="puedeAccion('donantes.crear')" class="form-card">
        ...
        <form @submit.prevent="createDonor">
          ...
          <div class="form-actions">
            ...
            <button type="button" class="btn btn-secondary" @click="loadDonantes" :disabled="isLoading">
              ...
              {{ isLoading && mode === 'list' ? 'Cargando...' : 'Listar donantes' }}
            </button>
          </div>
        </form>
      </article>
    ...
    <section v-if="showPanel" class="result-panel">
      <div class="result-head">
        <div class="result-head-info">
          <span class="label">Listado de donantes</span>
          <span v-if="resultKind === 'success' && Array.isArray(donantes)" class="count">
      ...
      <div v-if="resultKind === 'success'">
        <ul v-if="Array.isArray(donantes) && donantes.length" class="item-list">
          <li v-for="donante in donantes" :key="donante.id_donante" class="item-card">
            ...
            <div class="item-content">
              ...
              <div class="item-meta">
                <span class="badge badge-pink">{{ donante.tipo_donante }}</span>
              </div>
            </div>
            <div class="item-actions">
              <span class="badge badge-default">ID {{ donante.id_donante }}</span>
            </div>
          </li>
        </ul>

        <div v-else-if="Array.isArray(donantes) && donantes.length === 0" class="empty-list">
          <div class="icon">
          ...
          <p>Aún no se han creado donantes en la plataforma.</p>
        </div>

        <div v-else class="detail-card">
          <div class="detail-row">
            <span class="k">Detalle</span>
            <span class="v">Donante registrado correctamente.</span>
          </div>
        </div>
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadDonantes = async () => {
      ...
      mode.value = 'list'
      try {
        const response: Donante[] = await donanteService.list()
        resultKind.value = 'success'
    ...
    return {
      ...
      donantes, errorMessage,
      createDonor, loadDonantes, reset, closeResult,
